...
| **Async-signal-safe** | The small set of functions POSIX permits inside a signal handler — and, consequently, in the child of a `fork()` from a multi-threaded process. It excludes `printf`, `malloc`, and anything that takes a lock. | 10 |
...
| **Detached thread** | A thread declared with `pthread_detach()` as one nobody will `pthread_join()`. Its resources are reclaimed the moment it exits. **Every thread must be joined or detached**, or its bookkeeping is kept forever. | 10 |
...
| **`posix_spawn`** | ⭐ The POSIX call that creates a process running a named program in **one** step, duplicating nothing. QNX's preferred alternative to `fork()`+`exec()`, which has no good semantics in a multi-threaded process. `<spawn.h>`. | 10 |
...
| **Process** | A **container**, not a runner: an address space, file descriptors, a place in the pathname space, credentials, and a pid. It holds one or more threads and has no program counter of its own. **The process boundary is the boundary a fault cannot cross** (Ch 09). | 10 |
