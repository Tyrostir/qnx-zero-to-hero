...
| T-035 | 🟢 | `ls -l $QNX_TARGET/x86_64/usr/bin/sleep` and `ls $QNX_TARGET/x86_64/usr/bin \| head -30` | ⬜ | — | **Host only.** Settles [D-017](Doubts.md#d-017)'s open half: does the SDP ship `sleep`, or is it a `toybox` link? Also useful raw material for Ch 21's `mkifs` chapter. |
