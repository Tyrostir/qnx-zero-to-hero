...
| T-032 | 🔴 | **Run V15.1** — `make TGT=$TGT run` in `lab10_threads`, then `pidin -p threadzoo` | ⬜ | — | ⭐ **Decides whether Ch 10 §5.4 survives.** If `MUTEX`'s `Blocked` does not name the owning tid, §5.4 and Ch 12/25's diagnostic advice all change. |
| T-033 | 🟡 | **Run V15.4** — the `fork()`-in-a-multi-threaded-process experiment | ⬜ | — | Ch 10 §4.1 refuses to guess. Three outcomes, three different rewrites. |
| T-034 | 🟢 | **Run V15.2 and V15.3** — headers, and the default thread stack size | ⬜ | — | Host-only for V15.2. Replaces three reasoned claims and one gap with measurements. |
| T-035 | 🟢 | `ls $QNX_TARGET/x86_64/usr/bin \| head -30` and `find $QNX_TARGET -name 'toybox*'` | ⬜ | — | **Host only.** ✅ The `sleep` half is answered — it is **not** in the SDP ([D-017](Doubts.md#d-017)). What remains is the listing itself: it clears Ch 05 §2.2's `[UNVERIFIED]` and is **Chapter 21's source material** for what `mkifs` can pick from. |
| T-036 | 🔴 | **Chapter 21 must put `toybox` into the `mkifs` image deliberately** | ⬜ | — | Hazard **H-17**. An image built only from `$QNX_TARGET/x86_64/` boots with **no `ls`**. Write this into Ch 21 when it is drafted. |
...
| Version | Date | Change |
|---------|------|--------|
| 1.25 | 2026-09-01 | D-017 closed. T-035 narrowed; **T-036** added — Ch 21 must add `toybox` explicitly (H-17). |
| 1.24 | 2026-09-01 | D-017: T-035 added. |
| 1.23 | 2026-09-01 | Chapter 10 published. T-032/033/034 added for block V15. |
